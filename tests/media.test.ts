import { describe, expect, it } from "vitest";
import { MemoryMedia, isSafePhotoKey } from "../src/lib/media";
import { createApp } from "../src/server/app";
import { MemoryStore } from "../src/server/memory-store";

describe("media objects", () => {
  it("only serves photos/ keys", () => {
    expect(isSafePhotoKey("photos/og-jenhsunhuang.jpg")).toBe(true);
    expect(isSafePhotoKey("../secret")).toBe(false);
  });

  it("serves stored bytes from /api/media", async () => {
    const media = new MemoryMedia();
    await media.put("photos/og-jenhsunhuang.jpg", new Uint8Array([255, 216, 255]), {
      httpMetadata: { contentType: "image/jpeg" },
    });
    const app = createApp({
      store: new MemoryStore(),
      config: {
        origin: "http://localhost:5173",
        siteName: "workwithme.lol",
        adminEmails: [],
        paddleEnvironment: "sandbox",
        emailFrom: "board@workwithme.lol",
      },
      media,
    });
    const ok = await app.request("/api/media/photos/og-jenhsunhuang.jpg");
    expect(ok.status).toBe(200);
    expect(ok.headers.get("content-type")).toBe("image/jpeg");
    const missing = await app.request("/api/media/photos/nope.jpg");
    expect(missing.status).toBe(404);
  });
});
