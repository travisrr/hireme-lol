...
Standing rules for every agent that edits `travisrr/hireme-lol`. Live site: **workwithme.lol**.

## Ship every edit

After any code edit, commit and ship to production. Do not leave finished work sitting only on a feature branch.

Before you ship, confirm which branch production actually tracks. Check GitHub’s default branch and the live Cloudflare Worker. Do not guess. Today that is `main` — verify again next time. Deploy the branch production uses.
